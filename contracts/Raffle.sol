...
contract Raffle is VRFConsumerBaseV2, AutomationCompatible {
    ...
    function performUpkeep(bytes memory /*performData*/) external {
        ...
        s_raffleState = RaffleState.CALCULATING; // changing the state so that no one else can enter while the winner gets picked.
    ...
    function fulfillRandomWords(
        ...
        s_raffleState = RaffleState.OPEN; // re-open the raffle after one winner has been chosen
        s_players = new address payable[](0); // resetting the player array since the lottery is over now
        s_lastTimeStamp = block.timestamp;
